...
pub struct RelocationInfo {
    ...
    /// of the first segment command that appears in the file (not necessarily the one with the lowest address).
    /// For images with the `MH_SPLIT_SEGS` flag set, this is an offset from the virtual memory address of data of the first read/write segment command.
    pub r_address: i32,
